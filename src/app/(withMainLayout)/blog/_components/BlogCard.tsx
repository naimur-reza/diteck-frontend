    ...
    return (
        <Card className="overflow-hidden transition-all hover:shadow-md cursor-pointer" onClick={onClick}>
            <div className="relative min-h-56 w-full">
                <Image src={thumbnail || "/placeholder.svg?height=192&width=384"} alt={title} fill className="object-cover" />
            </div>

            <CardHeader className="pb-2">
                <Link href={`/blog/${_id}`}>
                    <h3 className="text-xl font-semibold line-clamp-1">{title}</h3></Link>
            </CardHeader>
            ...
            <CardContent>
                ...
                <div className="flex items-center gap-2">
                    <Avatar className="h-8 w-8">
                        ...
                        <AvatarFallback>{getInitials(author?.name || "Author Name")}</AvatarFallback>
                    </Avatar>
